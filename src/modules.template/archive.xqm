...
declare function archive:import($filename as xs:string, $archive as item()) {
    ...
    let $rename := if ($filename = $identifier||".zip") then true() else if (not($store-md instance of element(error)) and $identifier != "") then try { xmldb:rename($archive:repo-path, $filename, $identifier||".zip") } catch * {<error>Could not rename {$filename} to {$identifier}.zip. ({$err:code} , {$err:description}, {$err:value})</error>}else ()
    return 
