...
module namespace annotation = "http://www.oeaw.ac.at/acdb/cuneidb/annotations";
...
declare namespace cfdb = "http://www.oeaw.ac.at/acdb/cuneidb";
...
declare function annotation:new($data as xs:string, $tablet as element(tei:TEI), $surface-id as xs:string) as xs:string? {
    switch(true())
        ...
        default return
            ...
            let $glyph-exists := 
                if (exists($tablet//tei:zone[@ulx = $ulx][@uly = $uly][@lrx = $lrx][@lry = $lry]))
                then true()  
                else false()
            
            (: update the tablet if this glyph has not been already annotated. :)
            let $update := 
                if ($glyph-exists)
                then ()
                else (
            ...
            return (
                let $response := 
                    ...
                    else <cfdb:response><msg>This zone has been annotated already.</msg></cfdb:response>
    ...
    switch(true())
        ...
        default return
            ...
            let $update := (
                ...
                annotation:char($xdata, $tablet, $annotation-id),
...
declare function annotation:read($tablet as element(tei:TEI), $surface-id as xs:string, $annotation-id as xs:string, $filter as xs:string*) {
    ...
    let $data := 
        <annotation>
            ...
            <img>/exist/apps/cuneidb/$app-root/data/tablets/{$tablet-id||"/"||$glyph-img}</img>
...
declare %private function annotation:char($data as map(), $tablet as element(tei:TEI), $annotation-id as xs:string){
...
declare %private function annotation:note($tablet as element(tei:TEI), $annotation-id as xs:string) as element(tei:seg)?{
    let $note := $tablet//tei:note[@target = '#glyph_'||$annotation-id]
    return $note
};

declare %private function annotation:note($data as map(), $tablet as element(tei:TEI), $annotation-id as xs:string) as element(tei:seg)?{
...
declare function annotation:parseContext($context as xs:string, $reading as xs:string) as map() {
    ...
    then ()
        ...
        let $before := string-join($highlighted/fn:match[@is-reading='true']/preceding-sibling::*,''),
            $glyph := $highlighted/fn:match[@is-reading='true']/text(),
            $after := replace(string-join($highlighted/fn:match[@is-reading='true']/following-sibling::*,''),'^\*','')
